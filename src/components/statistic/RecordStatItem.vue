<template>
  ...
  <div class="records-stat__item" :class="{active: isShow}">
    <p class="index">{{ idx + 1 }}</p>
    <p class="count">
      <span>{{ record.countEmotions }}</span>
      <CurrencyIcon />
    </p>
    ...
    <button @click="showInfo" class="showInfo" :class="{active: isShow}"><i class="material-icons">keyboard_arrow_down</i></button>
    <button @click="$emit('deleteRecord', record.id)"><i class="material-icons">delete</i></button>
    <p class="loader" :class="classObject">
      <Loader/>
    </p>
  </div>
